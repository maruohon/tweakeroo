...
public enum FeatureToggle implements IHotkeyTogglable, IConfigValueNotifyable
{
    ...
    TWEAK_AFTER_CLICKER             ("tweakAfterClicker",                   false, "X,A", KeybindSettings.BOTH, "Enables a \"after clicker\" tweak, which does automatic right clicks on the just-placed block.\nUseful for example for Repeaters (setting the delay)."),
    ...
    TWEAK_HOTBAR_SLOT_CYCLE         ("tweakHotbarSlotCycle",                false, "X,C", KeybindSettings.BOTH, "Enables cycling the hotbar slot after each placed block, up to the set max slot number"),
    ...
    TWEAK_PLACEMENT_GRID            ("tweakPlacementGrid",                  false, "X,G", KeybindSettings.BOTH, "When enabled, you can only place blocks in a grid pattern, with a configurable interval"),
    TWEAK_PLACEMENT_LIMIT           ("tweakPlacementLimit",                 false, "X,M", KeybindSettings.BOTH, "When enabled, you can only place a set number of blocks per use/right click"),
