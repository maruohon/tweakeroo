...
public class RenderUtils
{
    ...
    public static void renderHotbarSwapOverlay(MinecraftClient mc, MatrixStack matrixStack)
    {
        ...
        if (player != null)
        {
            ...
            for (int row = 1; row <= 3; row++)
            {
                ...
                {
                    ItemStack stack = player.method_31548().getStack(row * 9 + column);
    ...
    public static void renderPlayerInventoryOverlay(MinecraftClient mc)
    {
        ...
        fi.dy.masa.malilib.render.InventoryOverlay.renderInventoryStacks(type, mc.player.method_31548(), x + slotOffsetX, y + slotOffsetY, 9, 9, 27, mc);
    }

    public static void renderHotbarScrollOverlay(MinecraftClient mc)
    {
        Inventory inv = mc.player.method_31548();
        final int xCenter = GuiUtils.getScaledWindowWidth() / 2;
        final int yCenter = GuiUtils.getScaledWindowHeight() / 2;
